...
. 從 URL 中選擇 OVA 檔案或瀏覽至其位置，然後選取 * 下一步 * 。
...
. 在 * 選擇名稱和資料夾 * 欄位中，輸入虛擬機器名稱並選擇其位置。如果選中標記爲 * 自定義此虛擬機的硬件 * 的複選框，則在到達 *Ready to Completed* 窗口之前，將啓用名爲 * 自定義硬件 * 的附加步驟。
