...
sidebar: sidebar 
keywords: upgrade 
summary: 當您從適用於 VMware vSphere 10.0 和 10.1 的 ONTAP 工具升級至 10.2 時、系統會建立一個還原磁碟區、並將所有必要的詳細資料記錄到還原磁碟區中。 
---
...
[role="lead"]
當您從適用於 VMware vSphere 10.0 和 10.1 的 ONTAP 工具升級至 10.2 時、系統會建立一個還原磁碟區、並將所有必要的詳細資料記錄到還原磁碟區中。如果您需要恢復組態、可使用恢復 Volume 來恢復 VMware vSphere 設定的 ONTAP 工具。如果您正在執行 VMware vSphere 10.2 的 ONTAP 工具修補程式層級升級、則會使用相同的還原磁碟區、並重新整理詳細資料。HA 和非 HA 部署均支援升級。
