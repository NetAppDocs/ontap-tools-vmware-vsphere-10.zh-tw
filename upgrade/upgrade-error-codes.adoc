...
| 084 | 將 JobManager 變更回正式作業失敗 | 請執行下列步驟以復原 / 完成升級。1.啟用 Diagnostic Shell 2.執行指令：_sudo perl /home/maint/scripts/post-deploy-upgrade.pl --postupgrade_ 3.檢查 /var/log/post-deploy-upgrade.log 中的日誌 
...
| 090 | 儲存後端和 vCenter 的憑證驗證失敗 | 檢查升級日誌和日誌檔案 /var/log/cert_validation_error.log 並重試升級。 
