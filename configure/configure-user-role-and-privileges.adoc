...
summary: 使用 ONTAP tools 和 ONTAP System Manager 中的 JSON 檔案來設定儲存設備後端的使用者角色和權限。 
---
...
[role="lead"]
使用 ONTAP tools for VMware vSphere 和 ONTAP System Manager，為儲存設備後端設定 ONTAP 使用者角色和 Privileges。您可以使用提供的 JSON 檔案指派角色、手動建立使用者和角色，或為非管理員帳戶套用所需的最低 Privileges。

.開始之前
* 從 ONTAP tools for VMware vSphere 下載 ONTAP Privileges 檔案，網址為 _\https://<ONTAPtoolsIP>:8443/virtualization/user-privileges/users_roles.zip_。此 zip 檔案包含兩個 JSON 檔案。設定 ASA r2 系統時，請使用 ASA r2 專用的 JSON 檔案。
+
...
.. 從下拉清單中選擇產品功能為 *VSC、VASA Provider 和 SRA*。
+
...
=== 非管理員全域範圍叢集使用者所需的最低 Privileges 清單

本頁列出非管理員全域範圍叢集使用者在沒有 JSON 檔案的情況下所需的最低權限。如果叢集處於本機範圍，請使用 JSON 檔案建立使用者，因為 ONTAP tools for VMware vSphere 在 ONTAP 上進行佈建時，僅擁有讀取權限是不夠的。
