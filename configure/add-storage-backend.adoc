...
將儲存後端新增至 ONTAP 叢集。對於 vCenter 作為相關 SVM 租戶的多租戶設定，請使用 ONTAP 工具管理員來新增叢集。將儲存後端新增至全域叢集之後，您必須將叢集與所需的 vCenter 租戶建立關聯。vCenter 租戶必須搭載所需的儲存虛擬機器（ Storage Virtual Machines ， SVM ）。這可讓 SVM 使用者配置 vVols 資料存放區。您可以使用 SVM 在 vCenter 中新增儲存設備。

若要將儲存後端新增至本機叢集，請使用 vCenter 伺服器上的 ONTAP 工具外掛程式。

link:../concepts/ontap-tools-concepts-terms.html["關鍵概念與詞彙"]提供本節所使用詞彙的更多資訊。
