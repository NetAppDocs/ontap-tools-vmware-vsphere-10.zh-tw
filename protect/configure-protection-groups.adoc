...
NOTE: Storage Replication Adapter（SRA）支援在一致性群組上具有一個自動容錯雙工類型同步關係和非同步關係 SnapMirror 的扇出。然而，不支援在一致性群組上具有兩個非同步 SnapMirror 關係的扇出，或在 volume 上的扇出 SnapMirror 關係。Vault 類型 SnapMirror 關係不包含在這些扇出限制內。SRA 中對一致性群組的支援僅限於 Unified 和 ASA r2 儲存系統上的 SAN 協定（iSCSI 和 Fibre Channel）。一致性群組不支援 NFS；如果將 NFS 資料存放區 volume 放入一致性群組，Discover Devices 作業將會失敗。
