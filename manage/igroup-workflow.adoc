...
summary: 了解 ONTAP tools 如何建立和管理 igroup 及匯出原則，以供資料存放區存取。 
---
= ONTAP tools 中的 Igroup 和匯出原則工作流程
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
利用這些資訊了解 ONTAP tools for VMware vSphere 如何建立和管理資料存放區存取的 igroup 和匯出原則。

在 ONTAP tools for VMware vSphere 9.x 中，igroup 的建立和管理採用扁平結構，vCenter 中的每個資料存放區都關聯到單一 igroup。這種模型限制了 igroup 在多個資料存放區之間的靈活度和重複使用性。ONTAP tools for VMware vSphere 10.4 引入了巢狀 igroup，其中 vCenter 中的每個資料存放區都關聯到一個父 igroup，而每個主機則連結到該父 igroup 下的子 igroup。您可以定義具有使用者自訂名稱的自訂父 igroup，以便在多個資料存放區之間重複使用，從而簡化 igroup 管理。請利用這些資訊，了解 ONTAP tools for VMware vSphere 如何透過 igroup 工作流程管理 LUN 和資料存放區。不同的工作流程會產生不同的 igroup 組態，如下列範例所示：
...
ONTAP tools for VMware vSphere 建立子 igroup host2Igroup 並將其新增至現有的父 igroup DS1Igroup。
...
ONTAP tools for VMware vSphere 9.x 版本不支援階層式 igroup。移轉至 10.3 或更高版本時，igroup 必須規範化為階層式結構。
...
從 ONTAP tools 10.5P2 開始，移轉的 igroup 在不再使用時會被刪除。在較早版本中，移轉的 igroup 永遠不會移除。請注意下列事項：

* 遷移後的 igroup 不能在不同的資料存放區中重複使用。
* 自訂 igroup （使用者定義）永遠不會被刪除，並且可以繼續重複使用。
...
當您從 ESXi 主機刪除或取下 NFS 資料儲存時，ONTAP tools for VMware vSphere 會從匯出原則移除該主機的 IP 位址。如果沒有其他主機使用該匯出原則，則會將其刪除。當您刪除 NFS 資料儲存時，如果該匯出原則未被任何其他資料儲存重複使用，ONTAP tools for VMware vSphere 會移除與該資料儲存相關聯的匯出原則。如果匯出原則被重複使用，則會保留主機 IP 位址，不會變更。當您刪除資料儲存時，如果 ONTAP 需要存取，ONTAP tools 會從匯出原則移除主機 IP 位址，並指派預設匯出原則。
