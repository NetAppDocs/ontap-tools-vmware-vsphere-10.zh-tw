...
[role="lead"]
如果您將資料儲存區從非 ONTAP tools 代管的環境移動到由其代管的環境，請了解 igroup 的運作行為。ONTAP tools for VMware vSphere 採用巢狀 igroup 模型，這有助於解釋較早版本與目前實作之間的行為差異。

從 ONTAP tools for VMware vSphere 10.4 開始，ONTAP tools 會自動建立並維護 ONTAP 和 vCenter 物件，從而簡化 VMware 資料中心環境中的資料存放區管理。從 ONTAP tools for VMware vSphere 10.5 P2 開始，ONTAP tools 也會在移轉的 igroup 不再使用時自動將其刪除。
