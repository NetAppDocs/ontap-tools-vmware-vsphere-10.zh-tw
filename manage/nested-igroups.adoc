...
[role="lead"]
如果您同時管理 ONTAP tools 虛擬機器和 ONTAP 儲存系統，了解 igroup 的行為至關重要，尤其是在將資料儲存區從非 ONTAP tools 管理的環境遷移到 ONTAP tools 管理的環境時。本頁說明 ONTAP tools for VMware vSphere 如何呈現和管理 igroup。這是概念性資訊，用於了解早期版本與目前巢狀 igroup 模型之間的行為差異。
...
如果在 ONTAP 系統和 VMware 環境中直接建立 igroup 和資料存儲，ONTAP tools 最初不會管理這些物件。這將導致 igroup 結構扁平化。
