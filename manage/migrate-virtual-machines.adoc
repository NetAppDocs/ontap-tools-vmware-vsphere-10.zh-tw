...
支援不包含記憶體的虛擬機器快照。如果虛擬機器的快照包含記憶體，則無法保護該虛擬機器。

您無法保護包含記憶體快照的未受保護虛擬機器。請先刪除記憶體快照，然後再為虛擬機器啟用保護。

對於使用 ASA r2 儲存類型的 Windows 虛擬機，虛擬機快照為唯讀。啟動虛擬機器時，VASA Provider 會根據唯讀快照建立 LUN 並啟用 IOPS。關閉虛擬機器時，VASA Provider 會刪除 LUN 並停用 IOPS。
