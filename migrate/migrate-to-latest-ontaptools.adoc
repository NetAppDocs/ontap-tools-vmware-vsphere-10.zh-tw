...
. 只有在觸發資料存放區探索工作（可能需要 30 分鐘才能完成）之後，才能在適用於 VMware vSphere 10.4 的 ONTAP 工具中看到從 ONTAP 工具移轉的 NFS 和 VMFS 資料存放區。驗證資料儲存庫是否在 VMware vSphere 外掛程式使用者介面頁面的 ONTAP 工具概覽頁面上可見。
