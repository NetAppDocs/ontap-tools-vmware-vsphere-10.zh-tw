...
.ONTAP 工具 IP 位址
ONTAP tools for VMware vSphere 10 會公開單一外部 IP 位址（以前稱為負載平衡器），該位址作為存取虛擬機器功能的閘道。您需要在初始組態期間提供此位址。ONTAP tools Manager 介面會使用此位址，且該位址也可用於直接存取 Swagger 文件頁面和 REST API。
